...
function observationTimeBucket(observedAt, { heartbeat = false } = {}) {
  const ms = observedAt instanceof Date ? observedAt.getTime() : new Date(observedAt).getTime();
  if (heartbeat) return Math.floor(ms / HEARTBEAT_MS);
  return 'state';
}
...
function buildObservationDedupeKey({
  ...
  const canonical = [
    ...
    observationTimeBucket(observedAt, { heartbeat }),
  ].join('|');
...
function shouldRecordObservation({
  ...
  if (hoursUntilSession != null && hoursUntilSession >= 0 && hoursUntilSession <= NEAR_TERM_HEARTBEAT_HOURS) {
    const lastAtMs = lastObservationAt ? new Date(lastObservationAt).getTime() : 0;
    if (observedAt.getTime() - lastAtMs >= HEARTBEAT_MS) {
      return { record: true, reasons: ['heartbeat'], heartbeat: true };
    }
  }
...
async function insertMarketObservation(supabase, built, sessionForMemory = null, { freshPriceFromCurrentRun = false } = {}) {
  ...
  try {
    ...
    if (sessionForMemory) {
      const snap = buildObservationSnapshot(sessionForMemory, { freshPriceFromCurrentRun });
