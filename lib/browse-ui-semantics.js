...
function trustedSlots(s) {
  const vm = availabilityView?.getSessionOccupancyViewModel?.(s)
    ?? availabilityView?.getSessionAvailabilityViewModel?.(s);
  if (vm?.hasTrustedSlots || vm?.hasTrustedCount) return vm.spotsLeft;
...
function countTrustedOpenSessions(sessions, isoDate, {
  ...
  return (sessions || []).filter((s) => (
    s.isoDate === isoDate
    && (availabilityView?.getSessionOccupancyViewModel
      ? availabilityView.getSessionOccupancyViewModel(s).isOpen
...
function buildLiveSidePresentation(s, nowMs = Date.now()) {
  ...
  const verifiedAt = vm?.verifiedAt || s.threshold_scanned_at || s.thresholdScanAt || null;
  ...
  const trusted = vm?.hasTrustedSlots || isThresholdTrusted(s);
  const remaining = trusted ? (vm?.spotsLeft ?? trustedSlots(s)) : null;
  ...
  if (fresh && trusted && remaining != null) {
    ...
    detailLine = verifiedAt
      ? (fresh
        ...
        : `Last verified ${remaining} spot${remaining === 1 ? '' : 's'} remaining · ${formatAgeMinutes(verifiedAge)}`)
      : 'Not verified';
  } else {
...
function computeLiveNowSummary(sessions, nowMs = Date.now()) {
  const standardSessions = (sessions || []).filter((s) => (
    liveSchedule?.isStandardWaveSession ? liveSchedule.isStandardWaveSession(s) : false
  ));
  const leftLive = liveSchedule?.pickLiveSessionForSide
    ? liveSchedule.pickLiveSessionForSide(standardSessions, 'left', nowMs)
    : null;
  const rightLive = liveSchedule?.pickLiveSessionForSide
    ? liveSchedule.pickLiveSessionForSide(standardSessions, 'right', nowMs)
    : null;
  ...
  if (!leftLive && !rightLive) {
    const next = liveSchedule?.findNextUpcomingStandardWaveSession
      ? liveSchedule.findNextUpcomingStandardWaveSession(standardSessions, nowMs)
      : findNextUpcomingSession(standardSessions, nowMs);
    return {
      mode: 'idle',
