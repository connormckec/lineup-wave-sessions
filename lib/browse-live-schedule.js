...
(function initBrowseLiveSchedule() {
  ...
  function isStandardWaveSession(session) {
    if (isLessonSessionForWindow(session)) return false;
    const wave = Number(session?.wave ?? session?.raw?.wave);
    if (wave === 1 || wave === 2) return true;
    const side = String(session?.waveSide || session?.wave_side || '').trim();
    return /^left wave$/i.test(side) || /^right wave$/i.test(side);
  }
  ...
  function resolveSessionWindow(session, timeZone = BOOKING_TZ) {
    ...
    const endLabel = resolveEndLabel(session, startLabel, tileRange);
    ...
    return {
      ...
      endLabel,
      timeZone,
      isStandardWave: isStandardWaveSession(session),
