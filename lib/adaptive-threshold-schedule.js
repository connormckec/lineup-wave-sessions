...
const INVENTORY_TARGET_MINUTES_GENERAL = [
  ...
  { maxHoursUntilStart: 14 * 24, targetMinutes: 1440 },
  { maxHoursUntilStart: Infinity, targetMinutes: 4320 },
];
...
const DEFAULT_HORIZON = supportedHorizon.resolveSupportedHorizon({ todayIso: '2026-01-01' });
const NEAR_TERM_DATE_SCAN_MAX_HOURS_GENERAL = DEFAULT_HORIZON.thresholdScanMaxHours;
const NEAR_TERM_DATE_SCAN_MAX_HOURS_WATCHED = DEFAULT_HORIZON.thresholdScanMaxHours;
...
function resolveThresholdScanHorizon({
  todayIso,
  scrapeWeeksAhead,
  effectiveWeeksAhead,
  maxBookingHorizonDays,
} = {}) {
  if (!todayIso) return DEFAULT_HORIZON;
  return supportedHorizon.resolveSupportedHorizon({
    todayIso,
    scrapeWeeksAhead,
    effectiveWeeksAhead,
    maxBookingHorizonDays,
...
function isDateWithinNearTermDateScan({
  isoDate,
  watched = false,
  todayIso,
  daysFromTodayFn,
  ...
  horizon = null,
  maxHoursGeneral = null,
  maxHoursWatched = null,
  latestSupportedDate = null,
} = {}) {
  const daysAhead = daysFromTodayFn(isoDate, todayIso);
  if (!Number.isFinite(daysAhead)) return false;
  if (daysAhead < 0) return watched;

  const resolvedHorizon = horizon || (todayIso ? resolveThresholdScanHorizon({ todayIso }) : DEFAULT_HORIZON);
  const maxHours = watched
    ? (maxHoursWatched ?? resolvedHorizon.thresholdScanMaxHours)
    : (maxHoursGeneral ?? resolvedHorizon.thresholdScanMaxHours);
  const latestDate = latestSupportedDate ?? resolvedHorizon.latestSupportedDate;
  if (latestDate && isoDate > latestDate) return false;

  if (hoursUntilStart != null && Number.isFinite(hoursUntilStart)) {
    if (hoursUntilStart < 0) return watched;
    return hoursUntilStart <= maxHours;
  }
...
function collectDueDateScanCandidates(sessions, {
  ...
  for (const session of sessions || []) {
    ...
    const nearTerm = isDateWithinNearTermDateScan({
      ...
      todayIso,
      daysFromTodayFn,
      hoursUntilStart: evalResult.hoursUntilStart,
    ...
    if (evalResult.hoursUntilStart != null) {
      bucket.earliestHoursUntilStart = bucket.earliestHoursUntilStart == null
        ? evalResult.hoursUntilStart
        : Math.min(bucket.earliestHoursUntilStart, evalResult.hoursUntilStart);
    }
