...
async function createDeliveriesForEvent(supabase, changeEvent, watches, {
  ...
  for (const watch of watches) {
    ...
    if (deliveryProvider === 'webpush') {
      const subscriptions = await listPushSubscriptions(watch.user_key);
      if (!subscriptions.length) {
        skipped.push({ watchId: watch.id, reason: 'no_push_subscriptions' });
        continue;
      }
      ...
      for (const sub of subscriptions) {
        ...
        if (error) {
          if (isUniqueViolation(error)) {
            skipped.push({ watchId: watch.id, subscriptionId: sub.id, reason: 'duplicate_delivery' });
            continue;
          }
          throw error;
        }
    ...
    if (deliveryProvider !== 'ntfy') {
      skipped.push({ watchId: watch.id, reason: 'provider_disabled' });
      continue;
    }
...
async function recordChangeEventAndDeliveries({
  ...
  const deliveryResult = await createDeliveriesForEvent(
    ...
    insertResult.event,
    watches,
    { resolveDestination, deliveryProvider, listPushSubscriptions },
  );
...
function classifyDeliveryFailure(result, { invalidDestination = false, vapidConfigError = false } = {}) {
...
async function updateDeliveryAfterAttempt(supabase, delivery, result, {
  ...
  const classification = classifyDeliveryFailure(result, { invalidDestination, vapidConfigError });
