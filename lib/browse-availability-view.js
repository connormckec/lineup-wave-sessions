...
(function initBrowseAvailabilityView() {
  ...
  function buildAvailabilityDotBarHtml(viewModel) {
    ...
    if (vm.isFull && capacity > 0) {
      let dots = '';
      for (let i = 0; i < capacity; i += 1) dots += '<span class="slot-dot"></span>';
      return `<div class="slot-bar">${dots}</div>`;
    }
    if (vm.hasTrustedCount && capacity > 0 && spotsLeft != null) {
