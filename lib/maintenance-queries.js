...
function buildSelectedDateDiagnostics(candidate, {
  ...
  const reason = selectionReason
    || (candidate.watchedDueCount > 0 ? 'watched_due_priority' : 'general_most_overdue');
  return {
    isoDate: candidate.isoDate,
    hoursUntilEarliestSession: candidate.earliestHoursUntilStart ?? representative?.hoursUntilStart ?? null,
    targetFreshnessMinutes: representative?.targetFreshnessMinutes ?? null,
    actualFreshnessMinutes: representative?.actualFreshnessMinutes ?? null,
