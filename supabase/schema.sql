...
-- Web Push device subscriptions (service_role only; see 202607252100_web_push.sql)
...
alter table notification_deliveries add column if not exists push_subscription_id uuid references push_subscriptions(id) on delete set null;

-- Append-only market observations (price + trusted inventory history)
